...
const Header = () => {
  ...
  const { isDark, isNeo, toggleTheme } = useTheme();
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between gap-2 sm:gap-3">
        ...
        <div className="hidden lg:flex items-center gap-4 shrink-0">
          ...
          <nav className="flex items-center gap-1 whitespace-nowrap">
            ...
            <button
              onClick={toggleTheme}
              className={`theme-toggle-btn ml-2 ${isNeo ? 'text-black hover:bg-black/10' : isDark ? 'text-yellow-400 hover:bg-white/10' : 'text-gray-600 hover:bg-gray-200/60'} rounded-full transition-colors`}
              aria-label="Cycle theme"
            >
              <Sun size={18} className="icon-sun" />
              <Moon size={18} className="icon-moon" />
              <Hexagon size={18} className="icon-neo" />
            </button>
        ...
        <div className="lg:hidden flex items-center gap-1.5 sm:gap-2 shrink-0">
          <button
            onClick={toggleTheme}
            className={`theme-toggle-btn ${isNeo ? 'text-black' : isDark ? 'text-yellow-400' : 'text-gray-600'}`}
            aria-label="Cycle theme"
          >
            <Sun size={18} className="icon-sun" />
            <Moon size={18} className="icon-moon" />
