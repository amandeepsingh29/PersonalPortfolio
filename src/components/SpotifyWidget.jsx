...
const SpotifyWidget = () => {
    ...
    return (
        <div className={`hidden md:flex items-center gap-2 px-3 py-1.5 rounded-full border text-xs font-mono-space tracking-wider transition-colors ${isDark
            ...
            <span className="flex gap-1.5 truncate max-w-[280px]">
                <span className={`opacity-70 ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>Listening to:</span>
                <span className={isDark ? 'text-white font-bold truncate' : 'text-gray-900 font-bold truncate'}>{song.title}</span>
                <span className="opacity-70 truncate">- {song.artist}</span>
            </span>
